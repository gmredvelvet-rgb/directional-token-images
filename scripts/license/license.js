/**
 * Directional Token Images — licence client.
 *
 * Patreon OAuth, JWT token management, device fingerprinting, periodic
 * heartbeat and the world-level licence flag.
 *
 * This licence is a *soft* gate: an unlicensed world keeps every feature, and
 * only receives a periodic trial reminder. Nothing here may ever block
 * functionality — see license-ui.js.
 *
...
/**
 * Localize a `DTI.License.*` key, interpolating `data` when given.
 * @param {string} key
 * @param {object} [data]
 * @returns {string}
 */
export const L = (key, data) => localize(`DTI.License.${key}`, data);
