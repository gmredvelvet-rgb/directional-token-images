/**
 * Directional Token Images — licence prompts and the free-trial reminder.
 *
 ...
 *
 * Unlike Velvet Journals this card carries no migration notice: that text
 * explains why a *previous* activation stopped validating, and a module that
 * has never been activated anywhere has no such history to explain.
 */
...
export default class LicenseUI {
  ...
  static show({ autoHide = false } = {}) {
    ...
    card.id = CARD_ID;
    card.className = `dti-license-card ${licensed ? "is-active" : "is-trial"}`;
  ...
  static #header(badge = "") {
    return `
      <header>
        ...
        ${badge ? `<span class="dti-license-badge">${badge}</span>` : ""}
      </header>`;
  }
  ...
  static #trialMarkupGM() {
    return `
      ...
      <p class="dti-license-status">${L("TrialIntro")}</p>
      <p class="dti-license-note">${L("TrialPitch")}</p>
      <button type="button" data-action="connect" class="dti-license-primary">
        <i class="fa-brands fa-patreon" aria-hidden="true"></i> ${L("Connect")}
      </button>
      <button type="button" data-action="code">${L("HaveCode")}</button>
      <button type="button" data-action="dismiss" class="dti-license-dismiss">${L("Later")}</button>`;
  }
  ...
  static #trialMarkupPlayer() {
    return `
      ${LicenseUI.#header(L("TrialBadge"))}
      <p class="dti-license-status">${L("TrialIntroPlayer")}</p>
      <p class="dti-license-note">${L("TrialPlayerNote")}</p>
      <button type="button" data-action="dismiss" class="dti-license-dismiss">${L("Close")}</button>`;
  }
  ...
  static #activeMarkup() {
    ...
    return `
      ${LicenseUI.#header()}
      <p class="dti-license-status">${status}</p>
      <p class="dti-license-note">${L("ReleaseHint")}</p>
  ...
  static async #enterCode(card, prefill = "", error = "") {
    ...
    const code = await foundry.applications.api.DialogV2.prompt({
      ...
      classes: ["directional-token-images", "dti-license-dialog"],
      content: `${notice}
